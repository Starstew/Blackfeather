...
var BFRL = BFRL || {
	...
	parseMonsterManual: function() {
		...
		for (var i in MonsterManual) {
			...
			this.npcManifest[i] = new Function('x','y','Being.call(this,x,y);');
			this.npcManifest[i].extend(Being);
	...
	parseArmory: function() {
		...
		for (var i in Armory) {
			...
			this.weaponManifest[i] = new Function('x','y','notObj','Weapon.call(this,x,y,notObj);');
			this.weaponManifest[i].extend(Weapon);
	...
	Game.prototype = {
		...
		drawVisibleMap: function() {
			...
			for (var key in this.fovMapCells) {
				...
				var y = parseInt(parts[1]);
				var fgcolor = "#fff";
				var bgcolor = (this.fovMapCells[key] == '' ? BFRL.settings.mapWallColor : BFRL.settings.mapFloorColor);
				BFRL.display.draw(x,y,this.map.cells[key],fgcolor,bgcolor);
